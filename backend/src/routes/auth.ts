...
router.post(
  ...
  async (req: Request, res: Response): Promise<Response> => {
    try {
      ...
      if (!signInRes.ok) {
        const errData = (await signInRes.json()) as {
        ...
        return res.status(401).json({
          status: "error",
          message:
            msg === "INVALID_PASSWORD" || msg === "EMAIL_NOT_FOUND"
      ...
      const userProfile = signInResult.success
        ? signInResult.user
  ...
  async (req: Request, res: Response): Promise<Response> => {
    try {
      ...
      appEvents.emit(AppEventType.AUTH_SIGNUP, {
        ...
        message:
          "Your account has been created successfully. Explore our DSFM analytics!",
        category: "system",
        priority: "medium",
      });
...
router.get("/user/:uid", verifyToken, async (req: Request, res: Response) => {
  try {
...
router.post(
  ...
  async (req: Request, res: Response): Promise<Response> => {
    try {
      ...
      let uid: string;
      const existingUid = await getAuth()
