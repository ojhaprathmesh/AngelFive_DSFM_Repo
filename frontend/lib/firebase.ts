...
try {
  ...
  if (firebaseConfig.apiKey && firebaseConfig.apiKey !== "test") {
    ...
    console.warn(
      "Firebase API Key missing or invalid. Skipping initialization. This is expected during build time if secrets are not injected.",
    );
    // Provide mock/nullish objects to prevent runtime errors if these are accessed during build
    app = {} as FirebaseApp;
    auth = {} as Auth;
  }
} catch (error) {
  console.error("Firebase initialization error:", error);
  // During build, we don't want to fail the entire build if firebase fails to init
  if (process.env.NODE_ENV === "production") {
    console.warn("Continuing build despite Firebase init failure...");
  } else {
    throw error;
  }
}
